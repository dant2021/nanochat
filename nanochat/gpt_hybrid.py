...
@dataclass
class HybridConfigV1(GPTConfig):
    """V1: Extends GPTConfig with hybrid-specific parameters."""
    # Hybrid architecture
...
class HybridGPTV1(nn.Module):
    """V1: H-layers see L-layer output (which still contains token embedding info)."""
    def __init__(self, config: HybridConfigV1, pad_vocab_size_to=64):
        """
...
# =============================================================================
# V2: L-encoder → H-layers → L-decoder architecture
# H-layers only see L-encoder output (latent space), never token embeddings
# =============================================================================

@dataclass
class HybridConfigV2(GPTConfig):
    """V2: L-encoder/L-decoder split with H-layers in pure latent space."""
    # L-encoder/decoder layers
    n_l_encoder_layers: int = 2    # L-encoder layers (token → latent)
    n_l_decoder_layers: int = 2    # L-decoder layers (latent → logits)

    # H-layers (operate in latent space only)
    n_h_layers: int = 4            # H-block layer count (full attention)
    local_window_size: int = 64    # L-layer attention window
    ...
    def __post_init__(self):
        """Set n_layer to total L-layers for Block compatibility."""
        total_l = self.n_l_encoder_layers + self.n_l_decoder_layers
        object.__setattr__(self, 'n_layer', total_l)


class HybridGPTV2(nn.Module):
    """V2: L-encoder → H-layers → L-decoder architecture.

    Key difference from V1: H-layers NEVER see token embeddings directly.
    They only see L-encoder output (intermediate latent representations).

    Architecture:
        tokens → wte → L-encoder (causal) → latent_seq
                                                ↓
                    H-layers (full attn on latent_seq + scratchpad)
                                                ↓
                    L-decoder (causal) + L-encoder residual → lm_head → logits
    """

    def __init__(self, config: HybridConfigV2, pad_vocab_size_to=64):
        super().__init__()
        self.config = config
        ...
        # Scratchpad latents (lives in latent space, same dim as L-encoder output)
        self.scratchpad = nn.Parameter(
            torch.randn(config.n_scratchpad, config.n_embd) * 0.02
        )

        # === L-Encoder: token embeddings → latent space ===
        self.l_encoder = nn.ModuleList([
            Block(config, layer_idx=i)
            for i in range(config.n_l_encoder_layers)
        ])

        # === H-Layers: operate purely in latent space ===
        self.h_blocks = nn.ModuleList([
            HBlock(config, layer_idx=i)
            for i in range(config.n_h_layers)
        ])

        # === L-Decoder: latent space → logits ===
        # Layer indices continue from encoder for value embedding pattern
        decoder_start_idx = config.n_l_encoder_layers
        self.l_decoder = nn.ModuleList([
            Block(config, layer_idx=decoder_start_idx + i)
            for i in range(config.n_l_decoder_layers)
        ])
        ...
        # Per-layer scalars (for all L-layers: encoder + decoder)
        total_l = config.n_l_encoder_layers + config.n_l_decoder_layers
        self.resid_lambdas = nn.Parameter(torch.ones(total_l))
        self.x0_lambdas = nn.Parameter(torch.zeros(total_l))

        # Value embeddings: alternating pattern across all L-layers
        kv_dim = config.n_kv_head * head_dim
        self.value_embeds = nn.ModuleDict({
            str(i): nn.Embedding(padded_vocab_size, kv_dim)
            for i in range(total_l) if has_ve(i, total_l)
        })

        # Window sizes for L-layers
        self.encoder_window_sizes = [(config.local_window_size, 0)] * config.n_l_encoder_layers
        self.decoder_window_sizes = [(config.local_window_size, 0)] * config.n_l_decoder_layers
    ...
    def init_weights(self):
        ...
        # Gate weights init to zero
        for block in list(self.l_encoder) + list(self.l_decoder):
            if block.attn.ve_gate is not None:
                torch.nn.init.zeros_(block.attn.ve_gate.weight)
    ...
    def forward(self, tokens: torch.Tensor, targets: torch.Tensor = None, loss_reduction='mean'):
        ...
        # =========================================
        x_embed = self.wte(tokens)  # [B, T, D]
        x_embed = norm(x_embed)
        x0_embed = x_embed  # Save for x0 residual in encoder

        # =========================================
        # STAGE 2: L-Encoder (tokens → latent)
        # =========================================
        cos_sin = (self.cos[:, :T], self.sin[:, :T])
        x = x_embed
        for i, block in enumerate(self.l_encoder):
            x = self.resid_lambdas[i] * x + self.x0_lambdas[i] * x0_embed
            ve = self.value_embeds[str(i)](tokens) if str(i) in self.value_embeds else None
            x = block(x, ve, cos_sin, self.encoder_window_sizes[i], kv_cache=None)

        # CRITICAL: Save L-encoder output for residual to decoder
        l_encoder_output = x  # [B, T, D] - this is the "latent" representation

        # =========================================
        # STAGE 3: H-Layers (latent space processing)
        # =========================================
        # H-layers process chunks with scratchpad, operating purely on l_encoder_output
        # They NEVER see token embeddings (x_embed) - only the latent representation
        ...
        for chunk_start in range(0, T, cfg.chunk_size):
            chunk_end = min(chunk_start + cfg.chunk_size, T)
            chunk = l_encoder_output[:, chunk_start:chunk_end]  # From L-encoder, NOT x_embed!
            chunk_len = chunk_end - chunk_start
            ...
            # H-blocks update chunk tokens AND new scratchpad (all in latent space)
            for h_block in self.h_blocks:
                h_context = h_block(h_context, chunk_start_idx, chunk_len, cfg.n_scratchpad)
            ...
            if accumulated_scratchpad is None:
                ...
                accumulated_scratchpad = torch.cat([accumulated_scratchpad, refined_scratch], dim=1)

        # =========================================
        # STAGE 4: Combine H-output with L-encoder residual
        # =========================================
        h_output_tokens = torch.cat(chunk_outputs, dim=1)  # [B, T, D] - token positions only

        # CRITICAL: Residual connection from L-encoder
        # L-decoder input = H-output + L-encoder output
        decoder_input = h_output_tokens + l_encoder_output  # [B, T, D]

        # =========================================
        # STAGE 5: L-Decoder (latent → logits)
        # =========================================
        x = decoder_input
        x0_decoder = decoder_input  # x0 for decoder is the combined signal
        decoder_offset = cfg.n_l_encoder_layers

        for i, block in enumerate(self.l_decoder):
            layer_idx = decoder_offset + i
            x = self.resid_lambdas[layer_idx] * x + self.x0_lambdas[layer_idx] * x0_decoder
            ve = self.value_embeds[str(layer_idx)](tokens) if str(layer_idx) in self.value_embeds else None
            x = block(x, ve, cos_sin, self.decoder_window_sizes[i], kv_cache=None)
        ...
        x = norm(x)
        logits = self.lm_head(x)
        logits = logits[..., :cfg.vocab_size]  # slice to remove padding
        logits = logits.float()  # switch to fp32 for logit softcap and loss computation
    ...
    def _compute_loss(self, logits, targets, reduction='mean'):
        """Compute loss. V2 only outputs at token positions (no scratchpad in output)."""
    ...
    def estimate_flops(self):
        ...
        # L-encoder attention FLOPs (local window)
        l_encoder_attn = cfg.n_l_encoder_layers * 12 * h * q * min(cfg.local_window_size, t)

        # L-decoder attention FLOPs (local window)
        l_decoder_attn = cfg.n_l_decoder_layers * 12 * h * q * min(cfg.local_window_size, t)

        # H-block attention FLOPs
        n_chunks = (t + cfg.chunk_size - 1) // cfg.chunk_size
        h_attn_flops = cfg.n_h_layers * n_chunks * cfg.n_scratchpad * 12 * h * q

        total_attn = l_encoder_attn + l_decoder_attn + h_attn_flops
        num_flops_per_token = 6 * (nparams - nparams_exclude) + total_attn
    ...
    def num_scaling_params(self):
        ...
        value_embeds = sum(p.numel() for p in self.value_embeds.parameters())
        lm_head = sum(p.numel() for p in self.lm_head.parameters())
        l_encoder = sum(p.numel() for p in self.l_encoder.parameters())
        l_decoder = sum(p.numel() for p in self.l_decoder.parameters())
        h_blocks = sum(p.numel() for p in self.h_blocks.parameters())
        scratchpad = self.scratchpad.numel()
        scalars = self.resid_lambdas.numel() + self.x0_lambdas.numel()
        total = wte + value_embeds + lm_head + l_encoder + l_decoder + h_blocks + scratchpad + scalars
        assert total == sum(p.numel() for p in self.parameters()), "Parameter count mismatch"
        return {
            'wte': wte,
            'value_embeds': value_embeds,
            ...
            'total': total,
            # For compatibility with base_train scaling calculations
            'transformer_matrices': l_encoder + l_decoder + h_blocks,
        }
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        # Separate out all parameters into groups
        l_encoder_params = list(self.l_encoder.parameters())
        l_decoder_params = list(self.l_decoder.parameters())
        h_block_params = list(self.h_blocks.parameters())
        matrix_params = l_encoder_params + l_decoder_params + h_block_params
        value_embeds_params = list(self.value_embeds.parameters())
        embedding_params = list(self.wte.parameters())
        lm_head_params = list(self.lm_head.parameters())
        resid_params = [self.resid_lambdas]
...
HybridConfig = HybridConfigV1
HybridGPT = HybridGPTV1
